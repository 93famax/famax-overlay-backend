...
export default async function handler(req, res) {
  ...
  try {
    ...
    while (!found && pages < 5) {
      ...
      if (data.error) return res.status(200).json({ stillUnlocked: false })

      for (const item of (data.items || [])) {
        const text = item.snippet.topLevelComment.snippet.textDisplay.toUpperCase()
        if (text.includes(token.toUpperCase())) { found = true; break }
      }
    ...
    if (!found) {
      ...
      await redis.del(unlockedKey)
    }

    return res.status(200).json({ stillUnlocked: found })
  } catch (e) {
